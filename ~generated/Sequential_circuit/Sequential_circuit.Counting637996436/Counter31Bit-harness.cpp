...
#endif
#include <iostream>
class Counter31Bit_api_t: public sim_api_t<VerilatorDataWrapper*> {
    public:
    Counter31Bit_api_t(VCounter31Bit* _dut) {
        dut = _dut;
        main_time = 0L;
        is_exit = false;
#if VM_TRACE
    ...
    void init_sim_data() {
        ...
        sim_data.outputs.push_back(new VerilatorCData(&(dut->io_seg7led_anodes)));
        sim_data.outputs.push_back(new VerilatorCData(&(dut->io_seg7led_decimalPoint)));
        sim_data.outputs.push_back(new VerilatorCData(&(dut->io_seg7led_cathodes)));
        sim_data.signals.push_back(new VerilatorCData(&(dut->reset)));
        sim_data.signal_map["Counter31Bit.reset"] = 0;
    }
#if VM_TRACE
...
static Counter31Bit_api_t * _Top_api;
...
int main(int argc, char **argv, char **env) {
    ...
    VCounter31Bit* top = new VCounter31Bit;
    std::string vcdfile = "~generated/Sequential_circuit/Sequential_circuit.Counting637996436/Counter31Bit.vcd";
