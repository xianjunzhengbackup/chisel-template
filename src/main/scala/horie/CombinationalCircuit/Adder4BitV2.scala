...
class Adder4BitV2 extends Module{
...
object Adder4BitV2Obj extends App{
    (new chisel3.stage.ChiselStage).execute(Array("--target-dir","generated/horie/CombinationalCircuit/Adder4Bit"),Seq(ChiselGeneratorAnnotation(()=>new Adder4BitV2 )))
      chisel3.iotesters.Driver.execute(args,()=>new Adder4BitV2)(c=>new Adder4BitV2Test(c))
}
class Adder4BitV2Test(c:Adder4BitV2) extends PeekPokeTester(c){
    for{
        a <- 0 to 7
        b <- 8 to 15
        carry <- 0 to 1
...
class AdderLED extends Module{
  val io = IO(new Bundle{
    ...
    val overflowLED = Output(UInt(1.W))
  })

  val seg7LED = Module(new Seg7LED1Digit)
  val adder = Module(new Adder4BitV2)
  adder.io.a :=io.a
  adder.io.b :=io.b
  adder.io.carryIn :=0.U(1.W)
